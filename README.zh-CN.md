...
> 这是一个面向英语学习的社区二创版本，基于 Zara Zhang 开发的 [YouTube Digest](https://github.com/zarazhangrui/youtube-digest)。当前 1.6.0 版本围绕真实 YouTube 视频建立了“观看、理解、跟读、积累、复习、精读导出”的完整学习流程。
...
- 根据学习者的“学习中、模糊、已掌握”记录，挑选并高亮接近个人水平的单词、搭配、地道表达和金句，改为点击按钮手动触发，打开视频本身不消耗 Token。
...
- 缓存同一视频的字幕、翻译和精读分析，重新打开时直接从缓存恢复高亮，不会再发起 API 请求。
- 通过英文、简体中文或中英双语 Overview 快速判断视频内容，切换语言不会产生新的 AI 请求。
- 直接读取 YouTube 原生字幕，并用 Chrome 本地模型翻译，日常“看视频 + 读双语字幕”的流程不产生任何 API 费用。
- 提供独立的字幕工具页面，可以上传 yt-dlp 下载的 `.srt` 字幕，复用同一套分段策略和 Chrome 本地翻译进行翻译和导出，全程不需要打开 YouTube。

## 1.6.0 版本说明

- 修复智能精读的触发方式：只有点击按钮才会调用 DeepSeek，打开视频、重新加载侧边栏、切换精读强度都不会再触发请求。
- 新增独立的字幕工具页面：上传 yt-dlp 下载的 `.srt` 字幕，用 Chrome 内置翻译在本机翻译并导出，全程不需要打开 YouTube。
- 字幕工具和侧边栏的导出面板都新增结构化 JSON 导出格式，以及通过浏览器打印实现的 PDF 导出，方便把逐字稿交给其他 agent 或工具使用。
- 在字幕来自原生抓取时，采集视频的作者和发布时间，并写入 JSON 导出结果。
- 侧边栏头部新增“字幕工具”按钮（与设置按钮并列），设置页也新增对应入口。
...
智能精读是唯一必然调用 DeepSeek 的功能，因此不会自动运行。打开视频、重新加载侧边栏、切换精读强度都不会发出任何请求。在 Transcript 页点击“智能精读”按钮，才会对当前逐字稿分析一次。

分析结果按“视频 + 精读强度 + 学习画像”三者组合缓存。再次打开同一视频时，高亮直接从缓存恢复，不产生新的请求，按钮上会显示恢复了多少项。把某个词标记为“已掌握”或“模糊”会有意改变学习画像，因此下一次分析会反映你当前的水平，而不是重复推荐同样的内容。

轻量、均衡、深度决定向模型索取的候选数量。深度模式产出最长，也最容易触及输出长度上限。万一分析被上限截断，已经完整返回的词条会被保留，而不是整次调用作废。

...
> 这是一个面向英语学习的社区二创版本，基于 Zara Zhang 开发的 [YouTube Digest](https://github.com/zarazhangrui/youtube-digest)。当前 1.5.0 版本围绕真实 YouTube 视频建立了“观看、理解、跟读、积累、复习、精读导出”的完整学习流程。
...
- 直接读取 YouTube 原生字幕，并用 Chrome 本地模型翻译，日常“看视频 + 读双语字幕”的流程不产生任何 API 费用。

## 1.5.0 版本说明

- 字幕改为直接读取当前 YouTube 视频页的原生字幕轨，获取逐字稿不再需要 Supadata Key 和额度。
- 字幕接口返回为空时，自动回退到 YouTube 自带的“显示字幕记录”面板读取，侧边栏实时显示进度。
- Supadata 降级为可选付费方案，需要在设置里勾选并保存 Key 之后才会启用。
- 字幕翻译默认改用 Chrome 内置的本地翻译模型，免费、无需 API Key，需要 Chrome 138 或更高版本。
- DeepSeek 翻译保留为设置项，两条路径之间不做静默回退，浏览器翻译失败时不会擅自消耗 DeepSeek Token。
- 首次翻译某种语言时显示语言包下载进度；Chrome 翻译不可用时给出明确提示并指向对应设置项。
